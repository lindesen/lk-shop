    ...
    <div id="myOrderItem">
        ...
        <div class="itemCenter">
            <div class="centerLeft">
                <img v-for="cartGoods in order.cart_shop[0].data.slice(0, 5)" :key="cartGoods._id" :src="cartGoods.small_image" alt="" width="30">
            </div>
            <div class="centerRight">{{order.cart_shop.length > 6 ? '…' : ''}}</div>
        </div>
        <div class="itemBottom">
            <van-cell title=" " :value="`共${order.cart_shop[0].data.length}份商品 实付¥${order.shop_price + order.dis_price}`"></van-cell>
        </div>
    </div>
</template>
